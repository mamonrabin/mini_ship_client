/* eslint-disable @typescript-eslint/no-explicit-any */
"use client";
import { Star } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import React, { useState } from "react";

const ProductCard = ({ item }: { item: any }) => {
  const [loading, setLoading] = useState(true);

  return (
    <div className="bg-white rounded-xl border border-[#262626]/20 cursor-pointer group">
      <Link href={`/product/${item.id}`}>
        <div className="p-2 lg:h-87.5">
          {/* Image */}
          <div className="relative flex items-center justify-center">
            
            {/* Loader */}
            {loading && (
              <div className="absolute inset-0 flex items-center justify-center bg-gray-100 rounded-md animate-pulse">
                <div className="w-8 h-8 border-4 border-gray-300 border-t-[#FF5722] rounded-full animate-spin"></div>
              </div>
            )}

            <Image
              src={item.thumbelImage}
              alt={item.title}
              width={300}
              height={300}
              onLoadingComplete={() => setLoading(false)}
              className={`rounded-t-md object-contain transition-transform duration-300 group-hover:scale-102 ${
                loading ? "opacity-0" : "opacity-100"
              }`}
            />

            {item.discount && (
              <p className="absolute top-2 -right-2 bg-[#FF5722] text-white text-xs font-bold px-2 py-1 rounded-l">
                {item.discount.toFixed(0)}%
              </p>
            )}
          </div>

          {/* Title */}
          <h2 className="pt-3 md:text-base text-xs line-clamp-2">
            {item.title}
          </h2>

          <div className="flex text-sm items-center md:gap-6 gap-4 pt-2">
            <p className="md:text-base text-sm text-[#1B5DD5] font-medium">
              ৳{item.price.toFixed(2)}
            </p>
            <p className="text-gray-600 line-through md:text-sm text-xs">
              ৳{item.mrpPrice.toFixed(2)}
            </p>
          </div>

          <div className="flex items-center gap-1 py-2">
            {item?.rating && (
              <div className="flex items-center gap-1">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    size={12}
                    className={
                      i < Math.round(item?.rating)
                        ? "fill-[#FF5722] text-[#FF5722]"
                        : "text-gray-300"
                    }
                  />
                ))}
              </div>
            )}

            {item?.selProduct && (
              <p className="text-sm">({item?.selProduct})</p>
            )}
          </div>
        </div>
      </Link>
    </div>
  );
};

export default ProductCard;
